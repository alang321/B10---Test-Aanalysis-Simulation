function [anglesBody, xPosBody, anglesFlap, xPosFlap, errorsx] = getRotationFromReference2dMovingFlapRmFirstCol(ref, medianList)
...
for i = 1:length(medianList)
    ...
    if n == 7 || n == 8
        if n == 8
            body = medianList{i}(:,[2:5]);
            flap = medianList{i}(:,[6:8]);
        else
            body = medianList{i}(:,[1:4]);
            flap = medianList{i}(:,[5:7]);
        end
