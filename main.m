...
snapshotListGroupedColDBScan = groupSnapshotDBScan(snapshotListReducedOutlier, 4, 4);
%snapshotListGroupedColDBScan = groupSnapshotDBScanAveragedCorePts(snapshotListReducedOutlier, 0, 4, 4);
%snapshotListGroupedColDBScan = groupSnapshotDBScanAveraged(snapshotListReducedOutlier, 1, 2.5, 7);
%snapshotListGroupedColDBScan = groupSnapshotDBScanAveragedCorePts(snapshotListReducedOutlier, 1, 2.5, 7);
...
[anglesBody, xBody, anglesFlap, xFlap, errorsx] = getRotationFromReference2dMovingFlapRmFirstCol(case0avg2, medianPerSnapshot);

relativeAngle = {};
for i = 1:length(anglesBody)
    relativeAngle{end+1} = anglesFlap{i} - anglesBody{i};
end

interactiveFinalPlot(xBody, anglesBody, xFlap, anglesFlap, relativeAngle, snapshot, medianPerSnapshot, snapshotListGroupedColDBScan, errorsx, false);
